...
USBSDK_API int sdk_loginDevice(IRNETHANDLE hHandle, HWND hWnd = NULL);
...
//parameter:
//p sdk_create return value
//connectType: connect way  iGetCurSel: deviceID   portIndx: Serial number usbIndex:usb number
//return value:
//>0: Success     <0:  fail
USBSDK_API int OpenDevice(IRNETHANDLE p, int connectType, int iGetCurSel, int portIndx, int usbIndex);
...
USBSDK_API int ReadHandle(IRNETHANDLE p, char* buf, char* cmd, int* pLen, int cmdsize);
...
/**
* @brief Get temp formula
* @param[in] p sdk_create return value
* @param[in] Gain mode 0:high 1:low
* @param[out] coefficient k
* @param[out] coefficient b
* @return 1:T1((temp+273.2)*10) 2:T2(high:(temp+273.2)*30-7000 low:(temp+273.2)*15+3300) 3:T3(k*(temp+273.2)+b)
*/
USBSDK_API int sdk_get_formulatype(IRNETHANDLE p, int gain, int* k, int* b);
